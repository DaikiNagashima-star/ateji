...
function App() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user) {
        const searches = JSON.parse(localStorage.getItem(`searches_${user.uid}`) || '[]');
        searches.unshift(name.trim());
        localStorage.setItem(`searches_${user.uid}`, JSON.stringify([...new Set(searches)].slice(0, 20)));
      }
    } catch (err) {
